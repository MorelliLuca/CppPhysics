...
public:
  ...
  Point(VA::Vector3d pos, VA::Vector3d vel) : position{pos}, velocity{vel} {}
  Point(VA::Vector3d pos) : position{pos} {}

  Point(double M) : mass{M} {}

  Point(double M, double ch) : mass{M}, charge{ch} {}

  auto const &getPosition() const { return position; }

  auto const &getVelocity() const { return velocity; }

  double getMass() const { return mass; }

  double getCharge() const { return charge; }

  double getSpeed() const { return mod(velocity); }

  double getKE() const { return mass * pow(this->getSpeed(), 2) / 2; }

  auto &accPosition() { return position; }

  auto &accVelocity() { return velocity; }
  ...
  void evolveDynamics(VA::Vector3d const &force) {
    position += velocity * VA::Const::epsilon +
                force / (2 * mass) * pow(VA::Const::epsilon, 2);
...
public:
  Gas(Point const &p, long unsigned int n, VA::Vector3d borders)
      : population(n, p), N{n}, containerSize{borders} {}
  Gas(std::vector<Point> pop, VA::Vector3d borders)
      : population{pop}, N{population.size()}, containerSize{borders} {}
  ...
  auto getPointer(int n = 0) { return &population[n]; }

  Point const &getParticle(int n) const { return population[n]; }

  Point &accParticle(int n) { return population[n]; }
  ...
  void evolveDynamics(VA::Vector3d externalForces) {
    ...
    for (auto &p1 : population) {
      ...
      for (auto &p2 : population) {
        internalForces += VA::hat(p2.getPosition() - p1.getPosition()) /
                          pow(distance(p1, p2), 2) *
                          (-1 * VA::Const::G * p1.getMass() * p2.getMass() +
                           VA::Const::K_e * p1.getCharge() * p2.getCharge());
      }
