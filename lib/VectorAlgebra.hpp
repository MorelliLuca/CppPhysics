...
template <typename T> struct Vector3 {
  ...
  // Overload @= operators
  template <typename S> Vector3<T> const &operator+=(Vector3<S> const &lhs) {
    x += lhs.x;
    y += lhs.y;
    z += lhs.z;
    return *this;
  ...
  template <typename S> T operator*=(Vector3<S> const &lhs) {

    return x * lhs.x + y * lhs.y + z * lhs.z;
  }

  // Conversion between types
  template <typename S> operator Vector3<S>() const {
    return Vector3<S>{x, y, z};
  }

#ifdef USING_SFML

  template <typename S> operator sf::Vector3<S>() {
    return sf::Vector3<S>{x, y, z};
  }
  template <typename S> operator sf::Vector2<S>() {
...
template <typename S> inline auto operator-(Vector3<S> const &lhs) {
  Vector3<S> zero{0, 0, 0};
  return zero -= lhs;
}

template <typename T> inline auto operator*(Vector3<T> rhs, double const &lhs) {
  return rhs *= lhs;
}

template <typename T> inline auto operator*(double const &rhs, Vector3<T> lhs) {
  return lhs *= rhs;
}
...
template <typename T> inline double mod(Vector3<T> arg) {
  return sqrt(arg * arg);
}

// Returns the versor v_hat given v
template <typename T> inline auto hat(Vector3<T> arg) { return arg / mod(arg); }

// Cross product
template <typename T, typename S>
inline Vector3<T> crossP(Vector3<T> rhs, Vector3<S> const &lhs) {
  return Vector3<T>{rhs.y * lhs.z - lhs.y * rhs.z,
